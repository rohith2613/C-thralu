...
import BackupImg from '../assets/clapperboard.png'
      ...
      <section className='flex justify-around flex-wrap py-5'>
        ...
        <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
         ...
         <p className='py-4'>{movie.overview}</p>
          {movie.genres ? (
          <p className='my-7 flex flex-wrap gap-2 text-center lg:text-left'>
          {movie?.genres?.map((genre) => (
          <span className='text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2' key={genre.id}>{genre.name}</span>
...
<div className="flex items-center">
    ...
    <p className="ms-2   text-gray-900 dark:text-white">{movie.vote_average
}</p>
